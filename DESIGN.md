...
- **配置编辑**：control / inbounds / outbounds / routing / dns 分区结构化编辑，并保留 Raw JSON
  全量编辑与无损格式化；两种模式双向同步，未知字段通过数组项的“其他字段 JSON”或 Raw 模式保留。
  保存统一走完整管线：`-check-config` → 备份 → 落盘 → `POST /v1/config/reload`
  （返回 `restart_required` 时改为 systemctl restart）→ `GET /v1/ready` 探活
